...
class Suppliers_with_ordersGeneralRetuen extends Controller
{
    public function index()
    {
        ...
        $suupliers = get_cols_where(new Supplier(), array('supplier_code', 'name'), array('com_code' => $com_code), 'id', 'DESC');
    ...
    {
        $com_code = auth()->user()->com_code;
        $suupliers = get_cols_where(new Supplier(), array('supplier_code', 'name'), array('com_code' => $com_code, 'active' => 1), 'id', 'DESC');
        $stores = get_cols_where(new Store(), array('id', 'name'), array('com_code' => $com_code, 'active' => 1), 'id', 'DESC');
    ...
    public function delete($id)
    {
        try {
            $com_code = auth()->user()->com_code;
            $parent_pill_data = get_cols_where_row(new Suppliers_with_orders(), array("is_approved", "auto_serial", "store_id", "supplier_code"), array("id" => $id, "com_code" => $com_code, 'order_type' => 3));
                        ...
                        if ($flagDelete) {
                            ...
                            if (!empty($itemCard_Data) and !empty($batch_data)) {
                                ...
                                if ($flag) {
                                    ...
                                    $SupplierName = get_field_value(new Supplier(), "name", array("com_code" => $com_code, "supplier_code" => $parent_pill_data['supplier_code']));
    ...
    public function delete_details($id, $parent_id)
    {
        try {
            $com_code = auth()->user()->com_code;
            $parent_pill_data = get_cols_where_row(new Suppliers_with_orders(), array("is_approved", "auto_serial", "store_id", "supplier_code"), array("id" => $parent_id, "com_code" => $com_code, 'order_type' => 3));
            if (empty($parent_pill_data)) {
                return redirect()->back()
    ...
    function do_approve($auto_serial, Request $request)
    {
        ...
        $data = get_cols_where_row(new Suppliers_with_orders(), array("total_cost_items", "is_approved", "id", "account_number", "store_id", "supplier_code"), array("auto_serial" => $auto_serial, "com_code" => $com_code, 'order_type' => 3));
        if (empty($data)) {
            return redirect()->route("admin.suppliers_orders.index")->with(['error' => "عفوا غير قادر علي الوصول الي البيانات المطلوبة !!"]);
        }
        if ($data['is_approved'] == 1) {
        ...
        $SupplierName = get_field_value(new Supplier(), "name", array("com_code" => $com_code, "supplier_code" => $data['supplier_code']));
    ...
    public function Add_item_to_invoice(Request $request)
    {
        try {
            if ($request->ajax()) {
                $com_code = auth()->user()->com_code;
                $invoice_data = get_cols_where_row(new Suppliers_with_orders(), array("is_approved", "order_date", "supplier_code"), array("com_code" => $com_code, "auto_serial" => $request->autoserailparent, 'order_type' => 3));
                                ...
                                if (!empty($itemCard_Data)) {
                                    ...
                                    if (!empty($flag_datainsert_items)) {
                                        ...
                                        if ($flag) {
                                            ...
                                            $SupplierName = get_field_value(new Supplier(), "name", array("com_code" => $com_code, "supplier_code" => $invoice_data['supplier_code']));
